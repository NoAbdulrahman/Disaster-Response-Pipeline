import sys
import pandas as pd
import numpy as np
from sqlalchemy import create_engine

def load_data(messages_filepath, categories_filepath):
    '''
    Input: 
    - the file path of messages dataset
    - the file path of categories dataset
    
    Output:
    - a dataframe contains the two datasets after merging
    '''
    
    messages = pd.read_csv('disaster_messages.csv')
    categories = pd.read_csv('disaster_categories.csv')
    df = messages.merge(categories)
    return df
    
def clean_data(df):
    '''
    Input:
    - the dataframe of the two merged datasets 
    
    Output:
    - the cleaned dataset as a dataframe
    '''
    
    categories = df['categories']
    categories= categories.str.split(pat=';',expand=True) 
    row = categories.iloc[0]
    category_colnames = row.apply(lambda x: x[:-2])
    categories.columns = category_colnames
    for column in categories:
        categories[column] = categories[column].apply(lambda x: x[-1])    
        categories[column] =  categories[column].astype(int)
    df.drop(['categories'], inplace=True, axis=1)
    df = pd.concat([df,categories], axis=1)
    df=df.drop_duplicates(subset ="id", keep = 'first') 
    return df

def save_data(df, database_filename):
    '''
    Input:
    - the cleaned dataset
    - a name for the new database
    
    Output:
    - a SQLite database
    '''
    
    engine = create_engine('sqlite:///' + database_filename)
    df.to_sql('cleaned_dataset', engine, index=False, if_exists='replace' )
    
def main():
    if len(sys.argv) == 4:

        messages_filepath, categories_filepath, database_filepath = sys.argv[1:]

        print('Loading data...\n    MESSAGES: {}\n    CATEGORIES: {}'
              .format(messages_filepath, categories_filepath))
        df = load_data(messages_filepath, categories_filepath)

        print('Cleaning data...')
        df = clean_data(df)
        
        print('Saving data...\n    DATABASE: {}'.format(database_filepath))
        save_data(df, database_filepath)
        
        print('Cleaned data saved to database!')
    
    else:
        print('Please provide the filepaths of the messages and categories '\
              'datasets as the first and second argument respectively, as '\
              'well as the filepath of the database to save the cleaned data '\
              'to as the third argument. \n\nExample: python process_data.py '\
              'disaster_messages.csv disaster_categories.csv '\
              'DisasterResponse.db')


if __name__ == '__main__':
    main()